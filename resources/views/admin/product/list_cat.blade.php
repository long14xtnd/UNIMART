            ...
            <div class="card">
                ...
                @endif
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php tablePostCategory($ProductCategorys) ?>
                        </tbody>
                    </table>
...
function tablePostCategory($ProductCategorys, $parent_id = 0, $char = ''){
    $t = 0; 
    foreach($ProductCategorys as $category => $item){
        $t++;
        if($item->parent_id == $parent_id ){
            ...
            echo '<tr>';
                ...
                echo '<td>';
                    echo '<a href="'.route('ProductCategory.edit',$item->id).' " class="btn btn-success btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
                            <a href="'.route('ProductCategory.delete',$item->id).'" onclick="return confirm(\'Xóa vĩnh viễn bản ghi này?\')" class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></a>';
                            
                        
                echo '</td>';
            echo '</tr>';    

            unset($ProductCategorys[$category]);

            tablePostCategory($ProductCategorys, $item->id, $char.' --');
        }
    }
}
